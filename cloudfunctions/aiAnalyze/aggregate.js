...
const getDateRangeForPeriod = ({ periodType = 'month', month = '' } = {}) => {
  const now = new Date();
...
const buildTrendSeries = (bills = [], startDate, endDate) => {
...
const buildBudgetForecast = ({ summary = {}, range = {}, currentBills = [] }) => {
  ...
  const monthDays = new Date(Number(range.endDate.slice(0, 4)), Number(range.endDate.slice(5, 7)), 0).getDate();
  ...
  return {
    ...
    pressure,
    confidence: range.periodType === 'month' ? 0.82 : 0.64,
    suggestion: pressure === 'high'
      ? '当前消费速度偏快，建议优先压低高频小额支出。'
      : '按当前节奏看，本期预算压力可控。',
...
const buildAnalysisSeed = ({ currentBills = [], previousBills = [], range = {} }) => {
  ...
  const trend = buildTrendSeries(currentBills, range.startDate, range.endDate);
  const previousTrend = buildTrendSeries(previousBills, range.prevStartDate, range.prevEndDate);
